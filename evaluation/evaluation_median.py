...
def compute_scale_and_shift(prediction, target, mask):
    ...
    tmp = (mask * prediction * prediction)
    is_nan = np.isnan(tmp)
    a_00 = np.sum(tmp[~is_nan])
    tmp = (mask * prediction)
    is_nan = np.isnan(tmp)
    a_01 = np.sum(tmp[~is_nan])
    a_11 = mask.sum()

    # right hand side: b = [b_0, b_1]
    tmp = (mask * prediction * target)
    is_nan = np.isnan(tmp)
    b_0 = np.sum(tmp[~is_nan])
    tmp = (mask*target)
    is_nan = np.isnan(tmp)
    b_1 = np.sum(tmp[~is_nan])
...
def prepare_depth_data(target, prediction, clip_distance, reg_factor=3.70378, ssi=False):
...
def add_to_metrics(idx, metrics, target_, prediction_, mask, event_frame = None, prefix="", rescale = False, debug = False, output_folder=None):
    if len(metrics) == 0:
...
if __name__ == "__main__":
    ...
    p_file = prediction_files[0]
    predicted_depth = np.load(p_file)
    print(predicted_depth[~np.isnan(predicted_depth)].mean())
    ...
    for idx in tqdm.tqdm(range(num_it)):
        ...
        target_depth, _ = prepare_depth_data(target_depth, predicted_depth, flags.clip_distance, ssi=flags.ssi)
        ...
        metrics = add_to_metrics(idx, metrics, target_depth, predicted_depth, depth_mask, event_frame=None, prefix="_", rescale=flags.rescale, debug=debug, output_folder=flags.output_folder)
        ...
        if use_event_masks:
            ...
            add_to_metrics(-1, metrics, target_depth, predicted_depth, event_mask, event_frame = event_frame, prefix="event_masked_", rescale=flags.rescale, debug=debug)
